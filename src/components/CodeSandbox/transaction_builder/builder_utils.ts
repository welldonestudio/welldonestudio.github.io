...
export const stringStructTag = new TxnBuilderTypes.StructTag(
...
export function serializeArg(
  ...
  if (argType instanceof TxnBuilderTypes.TypeTagVector) {
    // We are serializing a vector<u8>
    if (argType.value instanceof TxnBuilderTypes.TypeTagU8) {
...
export function argToTransactionArgument(
  ...
  if (
    argType instanceof TxnBuilderTypes.TypeTagVector &&
    argType.value instanceof TxnBuilderTypes.TypeTagU8
  ) {
    if (!(argVal instanceof Uint8Array)) {
